import React, { useEffect, useState } from "react";
import { Link, useNavigate, useParams } from "react-router-dom";
import { getOrganizerById, updateOrganizer } from "../../../api/organizer.js";

export const EditOrganizer = () => {
    const { id } = useParams();
    const navigate = useNavigate();

    const [loading, setLoading] = useState(true);
    const [error, setError] = useState("");

    const [form, setForm] = useState({
        OrganizerName: "",
        Email: "",
        PhNumber: "",
        Address: "",
        DeleteFlag: false,
    });

    useEffect(() => {
        const fetchOrganizer = async () => {
            try {
                const res = await getOrganizerById(id);
                const organizer = res.data.data;

                console.log(organizer);

                setForm(prev => ({
                    ...prev,
                    OrganizerName: organizer.OrganizerName || "",
                    Email: organizer.Email || "",
                    PhNumber: organizer.PhNumber || "",
                    Address: organizer.Address || "",
                    DeleteFlag: organizer.DeleteFlag || false,
                }));
            } catch (err) {
                console.error(err);
                setError("Failed to load organizer data");
            } finally {
                setLoading(false);
            }
        };

        fetchOrganizer();
    }, [id]);

    const handleOnChange = (e) => {
        const { name, value, type, checked } = e.target;

        if (type === 'checkbox') {
            setForm((prev) => ({ ...prev, [name]: checked }));
        } else {
            setForm((prev) => ({ ...prev, [name]: value }));
        }
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        setError("");

        try {
            // Prepare data - only send fields that have values (partial updates)
            const dataToSend = {};

            if (form.OrganizerName) dataToSend.OrganizerName = form.OrganizerName;
            if (form.Email !== undefined) dataToSend.Email = form.Email;
            if (form.PhNumber !== undefined) dataToSend.PhNumber = form.PhNumber;
            if (form.Address !== undefined) dataToSend.Address = form.Address;
            if (form.DeleteFlag !== undefined) dataToSend.DeleteFlag = form.DeleteFlag;

            // Add _method for Laravel if needed
            const dataWithMethod = {
                ...dataToSend,
                _method: "PUT" // Laravel-safe
            };

            console.log("Data to send:", dataWithMethod);

            const res = await updateOrganizer(id, dataWithMethod);
            console.log(res.data);

            navigate("/admin/organizers");
        } catch (err) {
            console.error(err);
            setError(err.response?.data?.message || "Failed to update organizer");
        }
    };

    if (loading) {
        return (
            <div className="px-4 sm:px-6 lg:px-8 py-6">
                <div className="mt-6 border border-gray-200 shadow-xl rounded-2xl overflow-hidden bg-white p-8 max-w-4xl mx-auto">
                    <div className="flex justify-center items-center h-64">
                        <div className="text-gray-500">Loading organizer data...</div>
                    </div>
                </div>
            </div>
        );
    }

    return (
        <div className="px-4 sm:px-6 lg:px-8 py-6">
            <div className="mt-6 border border-gray-200 shadow-xl rounded-2xl overflow-hidden bg-white p-6 md:p-8 max-w-4xl mx-auto">
                {/* Header */}
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-6 border-b border-gray-100">
                    <div className="mb-4 sm:mb-0">
                        <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Edit Organizer</h1>
                        <p className="text-gray-500 mt-2">Update organizer information in the system</p>
                    </div>
                    <Link
                        to="/admin/organizers"
                        className="flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 px-5 py-2.5 rounded-xl text-base font-medium transition-colors"
                    >
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
                        </svg>
                        Back to List
                    </Link>
                </div>

                {/* Error Message */}
                {error && (
                    <div className="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-lg">
                        {error}
                    </div>
                )}

                {/* Form */}
                <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {/* Organizer Name Field */}
                        <div className="space-y-2">
                            <label htmlFor="OrganizerName" className="block font-medium text-gray-700">
                                Organizer Name
                            </label>
                            <input
                                type="text"
                                id="OrganizerName"
                                name="OrganizerName"
                                value={form.OrganizerName}
                                onChange={handleOnChange}
                                placeholder="Enter organizer name"
                                className="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 focus:outline-none transition-colors"
                            />
                        </div>

                        {/* Email Field */}
                        <div className="space-y-2">
                            <label htmlFor="Email" className="block font-medium text-gray-700">
                                Email Address
                            </label>
                            <input
                                type="email"
                                id="Email"
                                name="Email"
                                value={form.Email}
                                onChange={handleOnChange}
                                placeholder="organizer@example.com"
                                className="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 focus:outline-none transition-colors"
                            />
                        </div>

                        {/* Phone Number Field */}
                        <div className="space-y-2">
                            <label htmlFor="PhNumber" className="block font-medium text-gray-700">
                                Phone Number
                            </label>
                            <input
                                type="tel"
                                id="PhNumber"
                                name="PhNumber"
                                value={form.PhNumber}
                                onChange={handleOnChange}
                                placeholder="+1 (555) 123-4567"
                                className="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 focus:outline-none transition-colors"
                            />
                        </div>

                        {/* Address Field - Full width */}
                        <div className="md:col-span-2 space-y-2">
                            <label htmlFor="Address" className="block font-medium text-gray-700">
                                Address
                            </label>
                            <textarea
                                id="Address"
                                name="Address"
                                value={form.Address}
                                onChange={handleOnChange}
                                rows="3"
                                placeholder="Enter organizer address..."
                                className="w-full border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500 focus:outline-none transition-colors"
                            />
                        </div>
                    </div>

                    {/* Info Box */}
                    <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
                        <div className="flex items-start gap-3">
                            <svg className="w-5 h-5 text-blue-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            <div>
                                <h4 className="text-blue-800 font-medium mb-1">Organizer Information</h4>
                                <p className="text-blue-700 text-sm">
                                    Update organizer information. All fields are optional - only update the fields you want to change.
                                    Email must remain unique if provided.
                                </p>
                            </div>
                        </div>
                    </div>

                    {/* Form Actions */}
                    <div className="flex flex-col sm:flex-row justify-end gap-4 pt-6 border-t border-gray-100">
                        <Link
                            to="/admin/organizers"
                            className="px-6 py-3 rounded-xl text-base font-medium text-gray-700 hover:bg-gray-50 border border-gray-300 transition-colors text-center"
                        >
                            Cancel
                        </Link>
                        <button
                            type="submit"
                            className="bg-purple-600 hover:bg-purple-700 text-white px-8 py-3 rounded-xl text-base font-semibold transition-colors shadow-sm hover:shadow-md"
                        >
                            Update Organizer
                        </button>
                    </div>
                </form>
            </div>
        </div>
    );
};